...
public class OneHandSword extends MeleeWeaponImpl {
    ...
    public void use(RPlayer rp) {
        ...
        try {
            if (b) {
                ...
                for (Entity e : Cone.getEntitiesInCone(p.getNearbyEntities(getCurrentWeaponRange(), 5, getCurrentWeaponRange()), p.getLocation().toVector(), getCurrentWeaponRange(), 45, p.getEyeLocation().getDirection())) {
                    LivingEntity le = (LivingEntity) e;
                    //damage
                    if (RTRPG.getInstance().rmobs.containsKey(le.getUniqueId())) {
                        CraftRMob mob = (CraftRMob) RTRPG.getInstance().rmobs.get(le.getUniqueId());
