...
public class Axe extends MeleeWeaponImpl {

    @Override
    public void use(RPlayer rp) {
        if (isCooldown()) return;
        Player p = rp.getPlayer();
        for (Entity e : Cone.getEntitiesInCone(p.getNearbyEntities(getCurrentWeaponRange(), 2, getCurrentWeaponRange()), p.getLocation().toVector(), getCurrentWeaponRange(), 180, p.getEyeLocation().getDirection())) {
